                                ...
                                const char *opts)
{
    ...
    for (token = buf; token && *token; token = next)
    {
        ...
        {
            if (arg)
                journal_flags |= strtoul(arg, &p, 0) ? EXT2_MKJOURNAL_LAZYINIT : 0;
            else
...
#define _SC_PAGESIZE _SC_PAGE_SIZE
     ...
     */
    fs_param.s_inodes_count = num_inodes ? num_inodes : (ext2fs_blocks_count(&fs_param) * blocksize) / inode_ratio;
...
int ext2fs_mkfs(io_manager manager, const char *filepath, int vsize)
{
    ...
    sprintf(opt_string, "tdb_data_size=%d", fs->blocksize <= 4096 ? 32768 : fs->blocksize * 8);
    ...
    free(hash_alg_str);
    fs->super->s_def_hash_version = (hash_alg >= 0) ? hash_alg : EXT2_HASH_HALF_MD4;
...
#define EXT2_DFL_CHECKINTERVAL (86400L * 180L)
    ...
    if (journal_device)
    {
        ...
        retval = ext2fs_open(journal_device, EXT2_FLAG_RW | EXT2_FLAG_JOURNAL_DEV_OK, 0,
